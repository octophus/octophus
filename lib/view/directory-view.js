...
module.exports = DirectoryView = (function (parent) {
  ...
  DirectoryView.prototype.initialize = function (directory) {
    ...
    self.on('mousedown', function (e) {
      ...
      var $selected = $('.octophus-view .selected');
      ...
      if ( (button === 0 || button === 2) && !(button === 2 && $selected.length > 1) ) {
        if ( !e[selectKey]) {
          ...
          $('.octophus-view .entries.list-tree').removeClass('multi-select');
        } else {
          $('.octophus-view .entries.list-tree').addClass('multi-select');
        }

...
export default class Directory {
	...
	createFile(path) {
		new_file = new File(this.repo, pathutil.join(this.path, path), pathutil.join(this.local_path, path), "100644", "<empty file>", this);
		this.files.push(new_file);
		new_file.save();
	}

	createDirectory(path) {
		new_directory = new Directory(this.repo, pathutil.join(this.path, path), pathutil.join(this.local_path, path), "040000", , this);

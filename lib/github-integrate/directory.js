		...
		let such_nice_promise = new Promise( (resolve, reject) => {
			...
			let such_magic_function = res => {
				res.tree.forEach(obj => {
					if (obj.type == "blob") {
						files.push(
							new File(this.repo, pathutil.join(this.path, obj.path), pathutil.join(this.tmppath, obj.path), obj.sha)
						)
					} else if (obj.type == "tree") {
						folders.push(
							new Directory(this.repo, pathutil.join(this.path, obj.path) + "/", pathutil.join(this.path, obj.path) + "/", obj.sha)
						)
					}
				})
				if (res.truncated) {

...
export default class File {
  ...
  open() {
  	if (this.sha == "<empty file>") return new Promise((resolve, reject) => {
  		fs.writeFile(this.local_path, "", err => {
  			if (err) reject(err);
  ...
  save(child) {
    ...
    setTimeout(this.repo.saveAll, atom.config.get('octophus.saveTimeout'));
  }

...
module.exports = Directory = (function (parent) {
  ...
  Directory.prototype.open = function (recursive, complete) {
    ...
    self.ghDir.open(recursive, function () {
      self.folders = [];
      self.files = [];

      self.ghDir.folders.forEach(function (ghFolder) {
        var name = path.basename(ghFolder.path);
        var dir = new Directory({
          parent: self,
          name: this.name,
          ghDir: ghFolder
        });
        self.folders.push(this.dir);
      });

      self.ghDir.files.forEach(function (ghFile) {
        var name = path.basename(ghFile.path);
        var file = new File({
          parent: self,
          name: this.name,
          ghFile: ghFile
        });
        self.files.push(this.file);
      });

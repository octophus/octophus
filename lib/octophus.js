  ...
  activate(state) {
    ...
    atom.commands.add('atom-workspace', {
      ...
      'octophus:open-repo': () => {
        ...
        dialog.on('open-repo', (e, fullrepo) => {
          ...
          let repo = fullrepo.split('/')[1].split(':')[0];
    ...
    atom.workspace.observeTextEditors((ed) => {
      ...
      const listener = buffer.onDidSave((ev) => {
        path = ev.path;
        if (atom.project.octophus.openfiles[path]) {
          atom.project.octophus.openfiles[path].save();
        }
      });
      const listener2 = buffer.onDidDestroy(() => {
        if (path && atom.project.octophus.openfiles[path]) {
          atom.project.octophus.repository.saveAll();
        }
        listener.dispose();
        listener2.dispose();
      });
